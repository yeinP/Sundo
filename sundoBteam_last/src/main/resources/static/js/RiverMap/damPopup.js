...
function fetchAndDisplayData(searchType, rfobscd, startD, endD) {
    ...
    var apiUrl2 = "http://api.hrfco.go.kr/8794F4D7-ED45-43EB-8E2F-1B721623F802/dam/info.json";
    console.log(apiUrl);
    ...
    $.get(apiUrl, function(result) {
        $.get(apiUrl2, function (result2) {
            ...
            $.each(result.content, function(index, value) {
                ...
                if(value.dmobscd === dmobscd) {
                    fldlmtwl = parseFloat(value.fldlmtwl);
                    pfh = parseFloat(value.pfh);
                }

            });

            var headers = result2.content;
            var targetDam = headers.find(function (dam) {
                return dam.dmobscd === dmobscd;
            });

            if (targetDam) {
                fldlmtwl = parseFloat(targetDam.fldlmtwl);
                pfh = parseFloat(targetDam.pfh);
            }
