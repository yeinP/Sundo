...
function fetchAndDisplayData(searchType, r_code, startD, endD) {
    ...
    var apiUrl2 = "http://api.hrfco.go.kr/52832662-D130-4239-9C5F-730AD3BE6BC6/waterlevel/info.json"
    ...
    $.get(apiUrl, function(result) {
        $.get(apiUrl2, function(result2) {
            ...
            var waterLevel = [];
            var waterFlowRate = [];
            var attwl =[]; // 관심 수위(단위:m)
            var wrnwl=[]; //	주의보 수위(단위:m)
            var	almwl=[];	//경보 수위(단위:m)
            var	srswl=[];	//심각 수위(단위:m)
            ...
            $.each(result.content, function(index, value) {
                ...
                table += "</tr>";
                if(value.wlobscd === r_code) {
                    attwl = parseFloat(value.attwl);
                    wrnwl = parseFloat(value.wrnwl);
                    almwl = parseFloat(value.almwl);
                    srswl = parseFloat(value.srswl);
                }

            });

            if (!isNaN(parseFloat(attwl))) additionalValues.push(parseFloat(attwl));
            if (!isNaN(parseFloat(wrnwl))) additionalValues.push(parseFloat(wrnwl));
            if (!isNaN(parseFloat(almwl))) additionalValues.push(parseFloat(almwl));
            if (!isNaN(parseFloat(srswl))) additionalValues.push(parseFloat(srswl));

            const validValues = [...waterLevel, ...waterFlowRate,...additionalValues].filter(value => !isNaN(value));
            const minYValue = Math.min(...validValues)-1;
            const maxYValue = Math.max(...validValues)+1;
            console.log('min: ------------------' + minYValue);
            var headers = result2.content;
            var targetRiver = headers.find(function (river) {
                return river.wlobscd === r_code;
            });
            if (targetRiver) {
                attwl = parseFloat(targetRiver.attwl);
                wrnwl = parseFloat(targetRiver.wrnwl);
                almwl = parseFloat(targetRiver.almwl);
                srswl = parseFloat(targetRiver.srswl);
            }
